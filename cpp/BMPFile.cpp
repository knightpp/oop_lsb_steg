...
void BMPFile::readFile()
{
	...
	assert(size > 0);
	auto* data = new unsigned char[size]; // allocate 3 bytes per pixel
	fread(data, sizeof(unsigned char), size, f); // read the rest of the pixels at once
	fclose(f);
...
void BMPFile::saveFile()
{
	...
	//
	// Headers done, now write the pixels...
	//
